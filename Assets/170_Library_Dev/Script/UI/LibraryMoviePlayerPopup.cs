...
{
	public class LibraryMoviePlayerPopup : PopupBase<SimplePopupResult>
    {
		// Definitions
        ...
        {
            LOG.Function(this, $"| parentSlots={parentSlots}");
            ...
            {
                addSlot().InitMoviePlayer(parentSlots[i].MovieList, parentSlots[i].Thumbnail, parentSlots[i].Date);
            }
        ...
        {
            var selectedSlots = slots.Where(slot => slot.Checked);
            int count = selectedSlots.Count();
            playBT.interactable = count > 0;
            countTMP.text = LocalizationMGR.One.GetText("WORD_131", count);
            var totalTicks = selectedSlots.Sum(slot =>
            {
                LOG.Important($"{slot.MovieList.PlayTime}", this);
                var timeSpan = TimeSpan.Parse($"00:{slot.MovieList.PlayTime}");
                return timeSpan.Ticks;
            });
            TimeSpan totalTimeSpan = new TimeSpan(totalTicks);
            timeTMP.text = LocalizationMGR.One.GetText("WORD_130", totalTimeSpan.Minutes);
