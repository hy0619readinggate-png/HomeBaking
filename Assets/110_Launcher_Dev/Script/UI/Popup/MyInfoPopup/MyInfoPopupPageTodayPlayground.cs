...
{
	public class MyInfoPopupPageTodayPlayground : MyInfoPopupSubBase
    {
        // Definitions
        // Properties

        // Methods
        public MyInfoPopupPageTodayPlayground()
        {
            category = "playground";
        }
        ...
        protected override async UniTask onLoad(CancellationToken cancellationToken, string date)
        {
            ...
            var result = await API.One.Call(UserData.One.Child.AccessToken, $"/api/v1/child/my-info/today/playground?searchDate={date}");
            ...
            if (result.Success)
            {
                ...
                int numFirstSlot = playgroundListAll.ToList().FindIndex(list => list.Value<int>("courseId") == UserData.One.Child.Course) + 1;
                var playgroundList = playgroundListAll.Where(list => list.Value<int>("courseId") == UserData.One.Child.Course).ToArray();
                ...
                courseTMP.text = $"Course {UserData.One.Child.Course}";
                ...
                for (int i = 0; i < playgroundList.Length; i++)
                {
                    ...
                    if (isComplete && idxCurrentSlot < i)
                        idxCurrentSlot = i;
                    slot.Init(UserData.One.Child.Course, numFirstSlot + i, stars, isComplete);
