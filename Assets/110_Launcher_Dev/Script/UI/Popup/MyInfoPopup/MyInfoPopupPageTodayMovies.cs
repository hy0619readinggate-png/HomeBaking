...
{
	public class MyInfoPopupPageTodayMovies : MyInfoPopupSubBase
    {
        // Definitions
        // Properties

        // Methods
        public MyInfoPopupPageTodayMovies()
        {
            category = "movies";
        }
        ...
        protected override async UniTask onLoad(CancellationToken cancellationToken, string date)
        {
            ...
            var result = await API.One.Call(UserData.One.Child.AccessToken, $"/api/v1/child/my-info/today/movies?searchDate={date}");
            ...
            if (result.Success)
            {
                ...
                foreach (var recordData in userContentList)
                {
                    ...
                    thumbnail.Init(contentName, sprite, true, 0);
