...
{
	public class MyInfoPopupPageTodayActivities : MyInfoPopupSubBase
    {
        // Definitions
        // Properties

        // Methods
        public MyInfoPopupPageTodayActivities()
        {
            category = "activities";
        }
        ...
        protected override async UniTask onLoad(CancellationToken cancellationToken, string date)
        {
            ...
            var result = await API.One.Call(UserData.One.Child.AccessToken, $"/api/v1/child/my-info/today/activities?searchDate={date}");
            ...
            if (result.Success)
            {
                ...
                foreach (var studyData in watchList)
                {
                    ...
                    LOG.Warning($"course={course}, type={type}", this);
