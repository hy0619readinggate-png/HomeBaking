...
{
	public class ParentChildLMSPopupPageDayEBooks : ParentChildLMSPopupPageDaySubBase
    {
        // Definitions
        // Properties

        // Methods
        public ParentChildLMSPopupPageDayEBooks()
        {
            category = "ebooks";
        }
        ...
        private void thumbnail_onClick(LMSContentThumbnail thumbnail)
        {
            ...
            UILauncher.One.ContentPU.ShowEbook(thumbnail);
        }
        ...
        protected override async UniTask onLoad(CancellationToken cancellationToken, string date)
        {
            ...
            var result = await API.One.Call(UserData.One.Parent.AccessToken, $"/api/v1/user/child-learning/day/{ChildData.ID}/ebooks?searchDate={date}");
            ...
            if (result.Success)
            {
                ...
                foreach (var recordData in userContentList)
                {
                    ...
                    var address = $"eBook/Thumbnail/Category{int.Parse(contentIndex.ToString().Substring(1, 2))}/{contentIndex}_w.png";
                    var sprite = await DataLoader.One.LoadSprite(address);
                    if (cancellationToken.IsCancellationRequested)
                        return;

                    thumbnail.Init(contentName, sprite, true, 1);
