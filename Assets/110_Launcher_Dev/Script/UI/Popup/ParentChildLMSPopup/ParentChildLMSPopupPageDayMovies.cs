...
{
	public class ParentChildLMSPopupPageDayMovies : ParentChildLMSPopupPageDaySubBase
    {
        // Definitions
        // Properties

        // Methods
        public ParentChildLMSPopupPageDayMovies()
        {
            category = "movies";
        }
        ...
        private void thumbnail_onClick(LMSContentThumbnail thumbnail)
        {
            ...
            UILauncher.One.ContentPU.ShowMovie(thumbnail);
        }
        ...
        protected override async UniTask onLoad(CancellationToken cancellationToken, string date)
        {
            ...
            var result = await API.One.Call(UserData.One.Parent.AccessToken, $"/api/v1/user/child-learning/day/{ChildData.ID}/movies?searchDate={date}");
            ...
            if (result.Success)
            {
                ...
                foreach (var recordData in userContentList)
                {
                    ...
                    var isRecorded = recordData.Value<bool>("isRecorded");

                    var address = $"Movie/Thumbnail/Category{int.Parse(contentIndex.ToString().Substring(1, 2))}/{contentIndex}.png";
                    var sprite = await DataLoader.One.LoadSprite(address);
                    if (cancellationToken.IsCancellationRequested)
                        return;

                    thumbnail.Init(contentName, sprite, true);
                    thumbnail.LogSN = logSn;
                    thumbnail.IsNew = isNew2;
                    thumbnail.HasRecordFiles = isRecorded;
        ...
        [SerializeField] private GameObject noUserContentGO;
        [SerializeField] private GameObject noWatchGO;
