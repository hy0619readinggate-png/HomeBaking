...
{
	public class ParentChildLMSPopupPageDayAIStudio : ParentChildLMSPopupPageDaySubBase
    {
        // Definitions
        // Properties

        // Methods
        public ParentChildLMSPopupPageDayAIStudio()
        {
            category = "ai-studio";
        }
        ...
        protected override async UniTask onLoad(CancellationToken cancellationToken, string date)
        {
            ...
            var result = await API.One.Call(UserData.One.Parent.AccessToken, $"/api/v1/user/child-learning/day/{ChildData.ID}/ai-studio?searchDate={date}");
