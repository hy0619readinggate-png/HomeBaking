...
namespace DoDoEng.Game.C3_G03
{
    public class StackType : MonoBehaviour
    {
        ...
        public void SetStackType(int addBlockCount)
        {
            ...
            // 1 : Stage에서 사용되는 위치
            // 2(tmplist) : 비어있으나 추가 블록이 들어갈 수 있는 위치
            for (int i = 0; i < rndStackType.Length; i++)
            {
                if (rndStackType[i] == "1")
                {
                    blockPf.Appear(typeIdx, emptyPosition[trIdx]);
            ...
            var addition = UtilArray.Random(0, typeIdx - 1, addBlockCount);

            for (int k = 0; k < addBlockCount; k++)
            {
                blockPf.Appear(typeIdx, tmpList[addition[k]]);
        ...
        public void SetBlock(List<string> restChunks, List<string> currChunks, SentenceData sd)
        {
            ...
            for (int j = 0; j < stagePosition.Count - currChunks.Count + selectedIdx.Count; j++)
            {
                ...
                if (j + currChunks.Count >= rndRestIdx.Length) restRnd = rndRestIdx[j + currChunks.Count - rndRestIdx.Length];
                else restRnd = rndRestIdx[j + currChunks.Count];
        ...
        IEnumerator crDrop(bool isDropped)
        {
            if (isDropped)
            {
                ...
                {
                    answerBox.Glow();
                    answerBox.SetText(C3_G03_Main.Instance.AnswerSentence);
