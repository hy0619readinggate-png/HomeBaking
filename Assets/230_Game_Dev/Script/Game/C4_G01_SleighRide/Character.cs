...
namespace DoDoEng.Game.C4_G01
{
	public class Character : MonoBehaviour
	{
			...
			while (gameObject && gameObject.activeSelf)
			{
				...
				if (wave.activeSelf && dodo.IsAnimationPlaying(CharacterAnimation.Move))
				{
					if (!bMoveAmb)
					{
						...
						C4_G01_Main.Instance.PlayAmbient(GetAudioClip("amb_slide_moves"));
						bMoveAmb = true;
					}
				}
				else
				{
					if (bMoveAmb)
					{
						C4_G01_Main.Instance.StopAmbient(GetAudioClip("amb_slide_moves"));
						bMoveAmb = false;
					}
				}
			}
		...
		void FixedUpdate()
		{
			...
			if (!C4_G01_Main.Instance.IsPlay) return;
...

			{
				...
				bgWaterPark.transform.Translate(dx / 3f, 0, 0);
				bgWater.transform.Translate(dx * 2f / 3f, 0, 0);
				bgWaterWave.transform.Translate(-dx, 0, 0);
			}
		...
		public void Jump()
		{
			Debug.Log($"★ Jump: {C4_G01_Main.Instance.IsPlay}, {bJump}, {curAni}");
			if (!C4_G01_Main.Instance.IsPlay) return;
			if (bJump) return;
			if (curAni == CharacterAnimation.Wrong) return;
			if (curAni == CharacterAnimation.Wrong2) return;
