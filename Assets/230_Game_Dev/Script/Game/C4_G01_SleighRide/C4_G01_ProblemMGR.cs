...
using GameData = DoDoEng.Common.GameData_C4_G01;

namespace DoDoEng.Game.C4_G01
{
    public class C4_G01_ProblemMGR : GameProblemMGR<GameData, ProblemData>
    {
        // Propertes
        ...
        protected override async UniTask<ProblemData[]> onBuild(GameList curriculum, GameData[] tables)
        {
            ...
            using (LOG.Coroutine($"Build() | {GameIDX}", this))
            {
                ...
                foreach (var config in config.RoundConfigs)
                {
                    ...
                    string[] phonetics = tables.Select(t => t.Word).Distinct().ToArray();
                    GameData[] tablesPhonetics = tables.Where(t => phonetics.Contain(t.Word)).ToArray();
                    ...
                    do
                    {
                        ...
                        {
                            QuizData[] A = datas.Where(t => t.Word == p).ToArray();
                            QuizData[] B = datas.Where(t => t.Word != p).OrderBy(x => random.Next()).ToArray();
                            QuizData[] words = new QuizData[]{A[0], B[0]};
