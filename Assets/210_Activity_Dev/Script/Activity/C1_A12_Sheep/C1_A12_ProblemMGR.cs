...
using ActivityData = DoDoEng.Common.ActivityData_C1_A12;

namespace DoDoEng.Activity.C1_A12
{
    public class C1_A12_ProblemMGR : ActivityProblemMGR<ActivityData, ProblemData>
    {
        // Definition
    ...
    public class ProblemData
    {
        ...
        public int[] SheepTypes;

        public override string ToString()
        {
            var exam = string.Join(",", Examples.Select(ex => ex.Text));
            var sheep = string.Join(",", SheepTypes);

            return $"<color=red>ProblemData" +
                $"[{Index} | {Text} | {exam} | {sheep}]" +
                $"</color>";
        }
    }
