...
namespace XDPaint.Tools.Image
{
    ...
    public sealed class BucketTool : BasePaintTool<BucketToolSettings>
    {
        ...
        [NonSerialized] private Color32[] filledPixels;
        [NonSerialized] private bool[] visitedPixels;
        ...
        public override void UpdateDown(PointerData pointerData)
        {
            ...
            var fillPosition = new Vector2Int((int)(pointerData.RaycastData.UVHit.x * textureSize.x), (int)(pointerData.RaycastData.UVHit.y * textureSize.y));
        ...
        public override void OnDrawProcess(RenderTargetIdentifier combined)
        {
            ...
            else if (isRunning && (!useThreads || (useThreads && !thread.IsAlive)))
            {
                ...
                if (Settings.UsePattern)
                {
                    ...
                    {
                        Data.CommandBuilder.Clear().LoadOrtho().SetRenderTarget(GetTarget(RenderTarget.CombinedTemp)).DrawMesh(Data.QuadMesh, Data.PaintMaterial, InputToPaintPass).Execute();
        ...
        private void FillTexture(Color32 fillColor, int width, int height, int x, int y)
        {
            ...
            var eps = Mathf.RoundToInt(Mathf.Clamp(Settings.Tolerance, 0f, 1f) * 65535f / 2f);
